...
describe("BackupBackendDetail — EMPTY_FORM defaults", () => {
    ...
    test("deletion_via_cron defaults to false", () => {
        expect(defaults.deletion_via_cron).toBe(false);
    });
...
describe("BackupBackendDetail — backupDst computation", () => {
    ...
    test("builds URL with bucket and path", () => {
        expect(backupDst({ s3_bucket: "my-bucket", s3_path: "backups" }))
            .toBe("boto3+s3://my-bucket/backups");
    });
    ...
    test("strips both leading and trailing slashes", () => {
        expect(backupDst({ s3_bucket: "my-bucket", s3_path: "/data/backups/" }))
            .toBe("boto3+s3://my-bucket/data/backups");
    });

    test("bucket only when path is empty", () => {
        expect(backupDst({ s3_bucket: "my-bucket", s3_path: "" }))
            .toBe("boto3+s3://my-bucket");
    });
    ...
    test("nested path is preserved", () => {
        expect(backupDst({ s3_bucket: "prod", s3_path: "a/b/c" }))
            .toBe("boto3+s3://prod/a/b/c");
    });
});
