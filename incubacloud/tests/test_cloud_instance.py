...
class TestCloudInstanceHealthCron(TransactionCase):
    ...
    def _restore_enqueue(self, original, model_class):
        # ``enqueue`` is inherited from the model class, so the patch added
        # it as an own attribute on the registry class. Odoo 18's test
        # teardown flags such leftovers as "unexpected attributes"; delete
        # it to fall back to the inherited method instead of reassigning.
        del model_class.enqueue
...
class TestCronRefreshBackupList(TransactionCase):
    ...
    def _restore_list_backups(self, original, model_class):
        # See _restore_enqueue: delete the own attribute the patch added so
        # Odoo 18's teardown doesn't flag it as an unexpected attribute.
        del model_class.list_backups
