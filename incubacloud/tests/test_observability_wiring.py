...
class TestEveryEmbedOffersAWayOut(BaseCase):
    """An embedded dashboard must always come with a link out of it.

    Measured against production on 16-ago-2026: the identity provider
    that Grafana delegates its sign-in to answers with
    ``content-security-policy: frame-ancestors 'self'``. A browser
    therefore refuses to paint that login page inside a frame served
    from another subdomain, so a visitor with no Grafana session yet
    gets a blank panel and no prompt — and nothing on screen says why.

    The cookie, which is what we had assumed was the obstacle, is not
    one: the panel and Grafana share a registrable domain, so a Lax
    cookie travels into the frame fine.

    Structural because there is no runtime signal to assert on: a frame
    blocked by CSP still fires ``load``, so the component cannot detect
    the case and offer the link only when it is needed. The link has to
    be there unconditionally, in every surface that embeds — which is
    exactly the kind of invariant that holds until someone adds a third
    surface.
    """

    def _templates(self):
        return sorted((_ROOT / "static" / "src").rglob("*.xml"))

    def test_every_template_that_embeds_also_links_out(self):
        embedding = [
            path for path in self._templates()
            if "ic-monitoring-frame" in path.read_text()
        ]
        self.assertTrue(
            embedding, "no dashboard embed found; this test lost its subject",
        )
        for path in embedding:
            self.assertIn(
                "ic-monitoring-escape", path.read_text(),
                f"{path.name} embeds a dashboard with no way to open it in "
                f"a tab of its own. Whoever has not signed in to Grafana "
                f"yet sees a blank frame there and cannot fix it from the "
                f"panel.",
            )

    def test_the_link_opens_a_new_tab(self):
        """Following it in place would navigate the SPA away to Grafana,
        losing the panel the user was reading."""
        for path in self._templates():
            body = path.read_text()
            if "ic-monitoring-escape" not in body:
                continue
            block = body.split("ic-monitoring-escape", 1)[1][:400]
            self.assertIn('target="_blank"', block, path.name)
            self.assertIn('rel="noopener"', block, path.name)

    def test_the_link_keeps_grafanas_own_chrome(self):
        """``kiosk`` hides the very sign-in prompt the tab is opened to
        answer, so the URL built for it must not carry the flag."""
        js = _MONITORING_JS.read_text()
        self.assertIn("externalUrl", js)
        self.assertIn("kiosk: false", js)
